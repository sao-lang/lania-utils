...
/**
 * @name exchangeChinese
 * @description 将一段文本转换成繁体或简体
 * @param {string} text 目标文本
 * @param {boolean} openSimple true-转成简体 false-转成繁体
 * @returns {string} 转换后的文本
 */
const exchangeChinese = (text: string, openSimple: boolean): string => {
    let result = '';
...
/**
 * @name convertToTraditional
 * @description 将一段中文转换成繁体
 * @param {string} content 目标文本
 * @returns {string} 转换后的文本
 */
export const convertToTraditional = (content: string) => {
    return exchangeChinese(content, false);
};
/**
 * @name convertToSimple
 * @description 将一段中文转换成简体
 * @param {string} content 目标文本
 * @returns {string} 转换后的文本
 */
export const convertToSimple = (content: string) => {
    return exchangeChinese(content, true);
};

/**
 * @name exchangeChineseInElement
 * @description 进行某个标签内部所有文本的简繁体转换
 * @param {boolean} openSimple true-转成简体 false-转成繁体
 * @param {HTMLElement} obj 标签元素，默认是body
 */
const exchangeChineseInElement = (
    openSimple: boolean,
    element: HTMLElement = document.body,
) => {
    const objs = element.childNodes;
    ...
    for (let i = 0; i < objs.length; i++) {
        ...
        if (nodeType === 3 && /[\u4e00-\u9fa5]/g.test(textContent as string)) {
            objs[i].textContent = exchangeChinese(
                textContent as string,
                openSimple,
            );
        } else if (nodeType === 1 && !stopDOM.includes(tagName)) {
            exchangeChineseInElement(openSimple, node as HTMLElement);
        }
    }
};
/**
 * @name convertToTraditionalInElement
 * @description 将某个标签内的中文转换成繁体
 * @param {HTMLElement} element 标签元素，默认是body
 */
export const convertToTraditionalInElement = (element = document.body) => {
    exchangeChineseInElement(false, element);
};

/**
 * @name convertToSimpleInElement
 * @description 将某个标签内的中文转换成简体
 * @param {HTMLElement} element 标签元素，默认是body
 */
export const convertToSimpleInElement = (element = document.body) => {
    exchangeChineseInElement(true, element);
};
